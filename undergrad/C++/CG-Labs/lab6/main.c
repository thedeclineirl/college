...
static float scale = 12.0;													//	scale for zooming									
...
//	Lighting Effects 
GLfloat light_position0[] = {  0.0f, -5.0f,  2.5f, 0.0f };
GLfloat light_position1[] = { -2.5f, -5.0f, -2.5f, 0.0f };
GLfloat light_position2[] = {  2.5f, -5.0f, -2.5f, 0.0f };

GLfloat ambient_light0[]        = { 0.2f, 0.0f, 0.0f, 1.0f };
GLfloat specular_light0[]       = { 0.5f, 0.0f, 0.0f, 1.0f };
GLfloat diffuse_light0[]        = { 0.2f, 0.0f, 0.0f, 1.0f };

GLfloat ambient_light1[]        = { 0.0f, 0.7f, 0.0f, 1.0f };
GLfloat specular_light1[]       = { 0.0f, 0.5f, 0.0f, 1.0f };
GLfloat diffuse_light1[]        = { 0.0f, 0.7f, 0.0f, 1.0f };

GLfloat ambient_light2[]        = { 0.0f, 0.0f, 0.2f, 1.0f};
GLfloat specular_light2[]       = { 0.0f, 0.0f, 0.5f, 1.0f };
GLfloat diffuse_light2[]        = { 0.0f, 0.0f, 0.2f, 1.0f };
...
static int nGridLines = 15;													//	size of ground-plane grid
...
void initInterface()														//	initialize the interface								
	...
	glutAddMenuEntry("Toggle Bone Axes", 'B');
	...
	glutAddMenuEntry("Toggle Recording", 'r');
	...
	/** Switch on lighting from render **/	
	InitLighting();
...
void printHelp()
	...
	printf("b:      Toggle black / grey background\n");
	...
	printf("l:      Cycle through light sources\n");
	printf("1/2:    Increase / decrease specular value for current light source\n");
	printf("3/4:    Increase / decrease ambient value for current light source\n");
	printf("5/6:    Increase / decrease diffuse value for current light source\n");
...
void keydown(unsigned char key, int x, int y)									//	what to do when a key is struck							
	...
	switch (key)															//	switch, based on the key								
		...
		case 'D':															//	dump parameters to output							
			...
			// dump lighting parameters
				
			// light position, specular, ambient and diffuse
			printf("\nRed Light 0 : \n");
			printf("Position { " );
			for(i = 0; i < 4; i++)	printf("%f  ", light_position0[i]);
			printf(" }\n");
			printf("Specular { " );
			for(i = 0; i < 4; i++)	printf("%f  ", specular_light0[i]);
			printf(" }\n");
			printf("Ambient { " );
			for(i = 0; i < 4; i++)	printf("%f  ", ambient_light0[i]);
			printf(" }\n");
			printf("Diffuse { " );
			for(i = 0; i < 4; i++)	printf("%f  ", diffuse_light0[i]);
			printf(" }\n\n");

			printf("\nGreen Light 1 : \n");
			printf("Position { " );
			for(i = 0; i < 4; i++)	printf("%f  ", light_position1[i]);
			printf(" }\n");
			printf("Specular { " );
			for(i = 0; i < 4; i++)	printf("%f  ", specular_light1[i]);
			printf(" }\n");
			printf("Ambient { " );
			for(i = 0; i < 4; i++)	printf("%f  ", ambient_light1[i]);
			printf(" }\n");
			printf("Diffuse { " );
			for(i = 0; i < 4; i++)	printf("%f  ", diffuse_light1[i]);
			printf(" }\n\n");

			printf("\nBlue Light 2 : \n");
			printf("Position { " );
			for(i = 0; i < 4; i++)	printf("%f  ", light_position2[i]);
			printf(" }\n");
			printf("Specular { " );
			for(i = 0; i < 4; i++)	printf("%f  ", specular_light2[i]);
			printf(" }\n");
			printf("Ambient { " );
			for(i = 0; i < 4; i++)	printf("%f  ", ambient_light2[i]);
			printf(" }\n");
			printf("Diffuse { " );
			for(i = 0; i < 4; i++)	printf("%f  ", diffuse_light2[i]);
			printf(" }\n\n");
		...
		case '1':
			// adjust the specular value for the current light
			printf(" light %d : specular -\n", light);
			switch(light){
				case 0:
					if(specular_light0[0] > 0.0f) specular_light0[0] -= 0.01f;
					glLightfv( GL_LIGHT0, GL_SPECULAR, specular_light0 );
					break;
				case 1:
					if(specular_light1[1] > 0.0f) specular_light1[1] -= 0.01f;
					glLightfv( GL_LIGHT1, GL_SPECULAR, specular_light1 );
					break;
				case 2:
					if(specular_light2[2] > 0.0f) specular_light2[2] -= 0.01f;
		...
		case '2':
			...
			switch(light){
				case 0:
					if(specular_light0[0] < 1.0f) specular_light0[0] += 0.01f;
					glLightfv( GL_LIGHT0, GL_SPECULAR, specular_light0 );
					break;
				case 1:
					if(specular_light1[1] < 1.0f) specular_light1[1] += 0.01f;
					glLightfv( GL_LIGHT1, GL_SPECULAR, specular_light1 );
					break;
				case 2:
					if(specular_light2[2] < 1.0f) specular_light2[2] += 0.01f;
					glLightfv( GL_LIGHT2, GL_SPECULAR, specular_light2 );
					break;
			}
			break;
		case '3':
			...
			switch(light){
				case 0:
					if(ambient_light0[0] > 0.0f) ambient_light0[0] -= 0.01f;
					glLightfv( GL_LIGHT0, GL_AMBIENT,  ambient_light0 );
					break;
				case 1:
					if(ambient_light1[1] > 0.0f) ambient_light1[1] -= 0.01f;
					glLightfv( GL_LIGHT1, GL_AMBIENT,  ambient_light1 );
					break;
				case 2:
					if(ambient_light2[2] > 0.0f) ambient_light2[2] -= 0.01f;
		...
		case '4':
			...
			switch(light){
				case 0:
					if(ambient_light0[0] < 1.0f) ambient_light0[0] += 0.01f;
					glLightfv( GL_LIGHT0, GL_AMBIENT,  ambient_light0 );
					break;
				case 1:
					if(ambient_light1[1] < 1.0f) ambient_light1[1] += 0.01f;
					glLightfv( GL_LIGHT1, GL_AMBIENT,  ambient_light1 );
					break;
				case 2:
					if(ambient_light2[2] < 1.0f) ambient_light2[2] += 0.01f;
					glLightfv( GL_LIGHT2, GL_AMBIENT,  ambient_light2 );
					break;
			}
			break;
		case '5':
			printf(" light %d : diffuse -\n", light);
			switch(light){
				case 0:
					if(diffuse_light0[0] > 0.0f) diffuse_light0[0] -= 0.01f;
					glLightfv( GL_LIGHT0, GL_DIFFUSE,  diffuse_light0 );
					break;
				case 1:
					if(diffuse_light1[1] > 0.0f) diffuse_light1[1] -= 0.01f;
					glLightfv( GL_LIGHT1, GL_DIFFUSE,  diffuse_light1 );
					break;
				case 2:
					if(diffuse_light2[2] > 0.0f) diffuse_light2[2] -= 0.01f;
		...
		case '6':
			...
			switch(light){
				...
				case 0:
					if(diffuse_light0[0] < 1.0f) diffuse_light0[0] += 0.01f;
					glLightfv( GL_LIGHT0, GL_DIFFUSE,  diffuse_light0 );
					break;
				case 1:
					if(diffuse_light1[1] < 1.0f) diffuse_light1[1] += 0.01f;
					glLightfv( GL_LIGHT1, GL_DIFFUSE,  diffuse_light1 );
					break;
				case 2:
					if(diffuse_light2[2] < 1.0f) diffuse_light2[2] += 0.01f;
					glLightfv( GL_LIGHT2, GL_DIFFUSE,  diffuse_light2);
					break;
			}
			break;
...
void display()//	routine that describes what to draw
{
	...
	else glClearColor(0.5, 0.5, 0.5, 1.0);	//	or maybe that was white	
...
void drawAxes()															//	render the axes of the coordinate system
	{ // drawAxes()
	int axisSize = (nGridLines > 5) ? 5 : nGridLines;
...
//	glTranslatef(-xVoxels+1, -yVoxels+1, -zVoxels+1);								//  offset to put axes in right place
	...
	glVertex3f(0.0, 0.0, 0.0);	glVertex3f(axisSize + 1.0, 0.0, 0.0);
	glVertex3f(0.0, 0.0, 0.0);	glVertex3f(0.0, axisSize + 1.0, 0.0);
	glVertex3f(0.0, 0.0, 0.0);	glVertex3f(0.0, 0.0, axisSize + 1.0);
	glEnd();
	...
	glMatrixMode(GL_MODELVIEW);												//	switch to modelview
	glPushMatrix();														//	push a matrix on the stack
	glTranslatef(0.0, 0.0, axisSize + 1.0);									//	move to end of z axis
	glutSolidCone(0.02*axisSize, 0.1*axisSize, 10, 2);							//	draw a solid cone for the z-axis
	glRotatef(90.0, 0.0, 1.0, 0.0);											//	rotate to draw in z=0 plane
	glTranslatef(0.0, 0.1*axisSize, 0.0);										//	move away slightly
	glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, blackColour);				//	edges don't reflect
	glMaterialfv(GL_FRONT_AND_BACK, GL_EMISSION, axisColour);						//	but they do emit
	glScalef(0.001*axisSize, 0.001*axisSize, 0.001*axisSize);					//	scale down
	...
	glPushMatrix();														//	push a matrix on the stack
	glTranslatef(0.0, axisSize + 1.0, 0.0);									//	move to end of y axis
	glRotatef(-90.0, 1.0, 0.0, 0.0);
	glutSolidCone(0.02*axisSize, 0.1*axisSize, 10, 2);							//	draw a solid cone for the z-axis
	glRotatef(90.0, 1.0, 0.0, 0.0);											//	rotate to draw in y=0 plane
	glTranslatef(0.1*axisSize, 0.0, 0.0);										//	move away slightly
	glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, blackColour);				//	edges don't reflect
	glMaterialfv(GL_FRONT_AND_BACK, GL_EMISSION, axisColour);						//	but they do emit
	glScalef(0.001*axisSize, 0.001*axisSize, 0.001*axisSize);					//	scale down
	...
	glPopMatrix();															//	pop matrix off stack
	glPushMatrix();														//	push a matrix on the stack
	glTranslatef(axisSize + 1.0, 0.0, 0.0);									//	move to end of x axis
	glRotatef(90.0, 0.0, 1.0, 0.0);
	glutSolidCone(0.02*axisSize, 0.1*axisSize, 10, 2);							//	draw a solid cone for the x-axis
...
void InitLighting(){
	/** TO DO:: Set up lights and components **/
		
	glLightfv(GL_LIGHT0,GL_POSITION,light_position0); //positions and sets colour of GL_LIGHT0
		
	glLightfv(GL_LIGHT0,GL_AMBIENT,ambient_light0);
	glLightfv(GL_LIGHT0,GL_DIFFUSE,diffuse_light0);
	glLightfv(GL_LIGHT0,GL_SPECULAR,specular_light0);
	glEnable(GL_LIGHT0); 	//enables the first light disable with glDisable(GL_LIGHT0);
	
	glLightfv(GL_LIGHT1,GL_POSITION,light_position1); //positions and sets colour of GL_LIGHT0
			
	glLightfv(GL_LIGHT1,GL_AMBIENT,ambient_light1);
	glLightfv(GL_LIGHT1,GL_DIFFUSE,diffuse_light1);
	glLightfv(GL_LIGHT1,GL_SPECULAR,specular_light1);
	glEnable(GL_LIGHT1); 	//enables the first light disable with glDisable(GL_LIGHT0);
		
	glLightfv(GL_LIGHT2,GL_POSITION,light_position2); //positions and sets colour of GL_LIGHT0
			
	glLightfv(GL_LIGHT2,GL_AMBIENT,ambient_light2);
	glLightfv(GL_LIGHT2,GL_DIFFUSE,diffuse_light2);
	glLightfv(GL_LIGHT2,GL_SPECULAR,specular_light2);
	glEnable(GL_LIGHT2); 	//enables the first light disable with glDisable(GL_LIGHT0);
